...
function printOutput(events) {
    ...
    for (let event of events) {
        ...
        if (event.type === "CreateEvent") {
            ...
            console.log(`Created a ${event.payload.ref_type} ${refLabel} on ${kleur.yellow(event.repo.name)} at ${kleur.green(formatTime(event.created_at))}`);
        }
        // DeleteEvent → branch / tag deleted
        if (event.type === "DeleteEvent") {
            const refLabel = event.payload.ref ? `"${kleur.red(event.payload.ref)}"` : "";
            console.log(`Deleted a ${event.payload.ref_type} ${refLabel} on ${kleur.yellow(event.repo.name)} at ${kleur.green(formatTime(event.created_at))})`);
        }
        ...
        if (event.type === "IssueCommentEvent") {
            console.log(`Comment ${event.payload.action} on an issue in ${kleur.yellow(event.repo.name)} at ${kleur.green(formatTime(event.created_at))}`);
        }
